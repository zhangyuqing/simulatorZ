...
\bioctitle[Short SimulatorZ]{SimulatorZ package vignette}
...
as shown in Figure~\ref{simulatorZ-vignette-cumhaz}.
...
for(i in 1:length(new_esets)){
    ...
    for(j in plotind){
      ...
      plot(exp(-survH.sim[[i]]*exp(lp[j])), type="l", 
           xlim=c(0, 3000), ylim=c(0, 1), xlab="time", ylab="probability of survival", main="estimated survival functions", col=color)
      par(new=TRUE)
    }    
  }
@
